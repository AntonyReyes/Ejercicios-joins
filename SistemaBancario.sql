...
create view v_clients_accounts as select c.id as "id cliente", c.first_name as nombre, c.last_name as apellido, c.email,a.account_no as cuenta,a.balance as saldo 
from clients c 
...
create view v_accounts_types as select a.account_no  as cuenta, c.id as cliente_id, at2.name
from clients c
right join accounts a 
on a.client_id = c.id 
full join account_types at2 
on at2.id = a.type;
...
create view v_transactions_detail as select t.id  as transacción, amount as monto, tt.name as "tipo de trasacción"
from transactions t 
inner join transactions_types tt 
on t.transaction_type = tt.id ;

select * from  v_transactions_detail;

-- 4 tipos_transaciones
create view v_transations_types_detail as
select tt.id as transacción, t.amount as monto, tt.name as "tipo de transación"
...
--5 las vistas y las consultas ya fuerón echas anteriormente
